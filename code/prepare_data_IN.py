...
def get_IN_DF(filename):
    ...
    return [df_confirmed, df_deceased, start, end]


def update():
    url_confirmed = 'https://api.covid19india.org/csv/latest/state_wise_daily.csv'
    filename_confirmed = '../data/raw/IN/time_series_covid19_confirmed_and_deaths.json'
...
def prepare():
    ...
    [df_confirmed, df_deceased, start, end] = get_IN_DF(filename)
