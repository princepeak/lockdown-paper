...
def get_CH_Confirmed_DF(filename):
    df = pd.read_csv(filename, delimiter=',')
    return get_resultant_DF(df)
...
def prepare():
    ...
    [df, start, end, dates] = get_CH_Confirmed_DF(filename_confirmed)
    df.to_csv('../data/processed/china/confirmed.csv', index=False)
    with open('../data/processed/china/confirmed.json', 'w', encoding='utf-8') as f:
        json.dump({'start':start, 'end': end, 'dates': dates}, f, ensure_ascii=False, indent=4)

    [df, start, end, dates] = get_CH_Death_DF(filename_deaths)
    df.to_csv('../data/processed/china/deaths.csv', index=False)
    with open('../data/processed/china/deaths.json', 'w', encoding='utf-8') as f:
        json.dump({'start':start, 'end': end, 'dates': dates}, f, ensure_ascii=False, indent=4)
